...
export const FRAMEWORK_VERSIONS: IFrameworkVersion[] = [
  {
    ...
    date: "2026-03-19",
    time: "15:00",
    label: "Cost Optimization + 15 Hook Formats + Sora2 4s Precision + Review Fixes",
    summary: "Switched creative batches from 3 calls to 1 call each (saves ~$2/gen). Added 15 proven UGC speaking hook formats. Rebuilt 4s Sora2 prompt spec for artifact prevention. Fixed demand temperature, overpromising, broad relatability.",
    changes: [
      { section: "Route", description: "Top Creatives: 3 parallel calls → 1 call. UGC text-overlay: 3 calls → 1 call. Saves ~$2 per generation.", type: "changed" },
      { section: "Route", description: "'Generate 5 More' also switched to single calls (regular + UGC in parallel)", type: "changed" },
      { section: "Hook Formats", description: "Added 15 proven UGC speaking formats: confession, wait-what, storytime, POV, urgency, unpopular opinion, reframe, de-influence, list, before-you-scroll, discovery, obsession, command, hot-take, rant", type: "added" },
      { section: "Hook Formats", description: "Formats added to all speaking mode prompts (sora2-bleed, handoff, silence) + topCreativesPrompt VO hooks", type: "added" },
      { section: "Sora2", description: "Rebuilt 4s text-overlay prompt spec: 10-point structure, exact camera/light/skin/clothing/object specs, mouth-closed rule, sensor noise markers", type: "changed" },
      { section: "Sora2", description: "Speaking precision fields (12) now referenced in topCreativesPrompt, not just audio mode prompts", type: "changed" },
      { section: "Sales Playbook", description: "Demand temperature: evaluate OUTCOME demand not mechanism demand", type: "fixed" },
      { section: "Prompts", description: "Added overpromising ban: hooks must not imply capabilities the feature doesn't have", type: "added" },
      { section: "Prompts", description: "Added broad relatability rule: hooks must hit 80%+ of target audience, not 0.1% niche", type: "added" },
      { section: "Prompts", description: "Creator Brief moved from buildContext to researchPrompt as privileged input", type: "changed" },
      { section: "Research", description: "Added viewerScenarios: 10-15 specific real-life moments where audience needs the product", type: "added" },
      { section: "Framework Data", description: "'The 2AM State' renamed to 'The Scroll-Stop State' — product-agnostic", type: "changed" },
    ],
  },
  {
    id: "v2.7",
    date: "2026-03-19",
    time: "12:30",
    label: "Creative Training Overhaul — Hook Energy, Anti-Slop, Speaking Precision",
    summary: "Rebuilt copy rules from user training session. Hook energy as variable (not just confession). 12 speaking precision fields for Sora2. Real speech guidance for VO scripts. Body text simplified to plain feature description.",
    changes: [
      { section: "Prompts", description: "Added hook energy dimension: discovery, hype, comedic, confrontational, FOMO, confession, product-voice — not just confession", type: "added" },
      { section: "Prompts", description: "Added 2-3 second test as first gate before any hook", type: "added" },
      { section: "Prompts", description: "Expanded AI slop detection with user's specific examples (banner ad text, church marketing, Netflix narration, generic excitement)", type: "changed" },
      { section: "Prompts", description: "Added 'what makes a hook good' section with real good/bad examples from user", type: "added" },
      { section: "Prompts", description: "Body text rule: plain feature in audience's words, not narrative/motivational/tagline", type: "changed" },
      { section: "Prompts", description: "Banned editorial questions, claims/conclusions, copywriter before/after, therapy vocabulary across ALL prompts", type: "changed" },
      { section: "Speaking UGC", description: "12 new speaking precision fields on IUGCPromptParams: eyeDirection, toneOfVoice, speakingPace, facialMicroExpression, bodyPosture, handBehavior, headMovement, pausePoints, voiceBreak, fillerWords, backgroundAmbience, breathingCue", type: "added" },
      { section: "Speaking UGC", description: "Real speech guidance: filler words by generation (zoomer vs millennial vs older), messy structure, mid-thought discovery", type: "added" },
      { section: "Speaking UGC", description: "Sora2 speaking prompts increased to 8-12 sentences with all precision fields woven in", type: "changed" },
      { section: "Speaking UGC", description: "Bad vs good VO script examples showing copywriter-read-aloud vs real person talking", type: "added" },
    ],
  },
  {
    id: "v2.5",
